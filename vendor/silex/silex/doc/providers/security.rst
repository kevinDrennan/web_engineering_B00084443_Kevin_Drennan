...
error and the last collegeId entered by the user in case of an authentication
error.
...
store the users::
    ...
    class UserProvider implements UserProviderInterface
    {
        ...
        public function loadUserByUsername($collegeId)
        {
            $stmt = $this->conn->executeQuery('SELECT * FROM users WHERE collegeId = ?', array(strtolower($collegeId)));

            if (!$user = $stmt->fetch()) {
                throw new UsernameNotFoundException(sprintf('Username "%s" does not exist.', $collegeId));
            }

            return new User($user['collegeId'], $user['password'], explode(',', $user['roles']), true, true, true, true);
        }
...
sample users::
    ...
    if (!$schema->tablesExist('users')) {
        ...
        $users->addColumn('collegeId', 'string', array('length' => 32));
        $users->addUniqueIndex(array('collegeId'));
